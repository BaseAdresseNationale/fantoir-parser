...
function parseRecord(record, {accept, parseDate}) {
  // EOF
  ...
  if (!isAlphaNumeric(record.charAt(6))) {
    if (accept.includes('commune')) {
      return parseCommune(flatten(record), parseDate)
    }
  ...
  if (accept.includes('voie')) {
    return parseVoie(flatten(record), parseDate)
  }
}
...
function createParser(options = {}) {
  ...
  return pumpify.obj(
    ...
    through.obj(function (chunk, enc, cb) {
      const parsedValue = parseRecord(chunk, {accept, parseDate})
      if (parsedValue) {
